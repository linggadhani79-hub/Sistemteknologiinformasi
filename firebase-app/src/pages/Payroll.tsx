...
import { PageHeader, Spinner, fmtRupiah } from '../components/ui';
...
export function Payroll() {
  ...
  const load = async (p: string) => {
    setLoading(true);
    const snap = await getDocs(query(collection(db, 'payroll'), where('periode', '==', p)));
    const list = await Promise.all(snap.docs.map(async (d) => {
      const pr = d.data();
      const pegSnap = await getDoc(doc(db, 'pegawai', pr.pegawaiId));
      return { id: d.id, ...pr, pegawai: pegSnap.data() };
    }));
    setRows(list);
    setLoading(false);
  };
  ...
  const generate = async () => {
    setBusy(true);
    try {
      const payrollGenerate = httpsCallable(functions, 'payrollGenerate');
      await payrollGenerate({ periode });
      load(periode);
    } finally { setBusy(false); }
  };

  const total = rows.reduce((s, r) => s + r.totalGaji, 0);
